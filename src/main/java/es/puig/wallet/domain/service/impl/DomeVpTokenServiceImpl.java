...
public class DomeVpTokenServiceImpl implements DomeVpTokenService {
    ...
    private Mono<VcSelectorRequest> buildVpTokenResponse(String processId, String authorizationToken, AuthorizationRequest authorizationRequest) {
        ...
        boolean scopeMatches = ApplicationConstants.DEFAULT_SCOPE_FOR_DOME_VERIFIER.stream().anyMatch(scope -> authorizationRequest.scope().contains(scope));

        if (scopeMatches) {
            // If there is a match, use DEFAULT_VC_TYPES_FOR_DOME_VERIFIER as the list of VC types
            return attestationExchangeCommonWorkflow.getSelectableCredentialsRequiredToBuildThePresentation(processId, authorizationToken, ApplicationConstants.DEFAULT_VC_TYPES_FOR_DOME_VERIFIER)
