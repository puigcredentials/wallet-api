...
import es.puig.wallet.domain.service.VerifierValidationService;
...
public class VerifierValidationServiceImpl implements VerifierValidationService {
    ...
    private Mono<SignedJWT> validateClientIdClaim(String processId, SignedJWT signedJWTAuthorizationRequest) {
        ...
        String iss = jsonPayload.get(ApplicationConstants.ISSUER_TOKEN_PROPERTY_NAME).toString();
        String sub = jsonPayload.get(ApplicationConstants.ISSUER_SUB).toString();
    ...
    private ECPublicKey decodeDidKey(String didKey){
        if (!didKey.startsWith(ApplicationConstants.DID_KEY_PREFIX)) {
            throw new IllegalArgumentException("Invalid DID Key format");
        }

        String encodedMultiBase58 = didKey.substring(ApplicationConstants.DID_KEY_PREFIX.length());
